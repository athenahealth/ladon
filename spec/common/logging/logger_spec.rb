...
module Ladon
  module Logging
    RSpec.describe Logger do
      ...
      describe '#level=' do
        let(:initial_level) { Level::ERROR }
        let(:new_level) { Level::FATAL }
        let(:logger) { Logger.new(level: initial_level) }
        subject { -> { logger.level = new_level } }

        before { logger.error('Some message') }

        it { is_expected.not_to change(logger, :entries) }

        it { is_expected.to change(logger, :level).from(initial_level).to(new_level) }
      end
      ...
      describe 'to_ methods' do
        ...
        describe '#to_h' do
          subject { -> { logger.to_h } }
          let(:expected_hash) { { level: level, entries: [] } }
